...
class CartModel {
    ...
    public function getByUser($user_id) {
        $db = getDB();
        $stmt = $db->prepare('SELECT c.*, p.name, p.price, p.discount, p.image_url FROM carts c JOIN products p ON c.product_id = p.id WHERE c.user_id = :uid');
    ...
    public function getTotal($user_id) {
        $db = getDB();
        $stmt = $db->prepare('SELECT SUM((p.price - p.discount) * c.quantity) AS total FROM carts c JOIN products p ON c.product_id = p.id WHERE c.user_id = :uid');
        $stmt->execute(['uid' => $user_id]);
