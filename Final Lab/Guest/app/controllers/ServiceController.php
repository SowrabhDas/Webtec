...
class ServiceController {
    ...
    public function estimate() {
        ...
        $service_slug = $_POST['service'] ?? '';
        $vehicle = trim($_POST['vehicle'] ?? '');
        $year = intval($_POST['year'] ?? 0);

        $errors = [];
        if (empty($service_slug)) $errors[] = 'Service required';
        if (empty($vehicle)) $errors[] = 'Vehicle required';
        if ($year <= 1900 || $year > intval(date('Y')) ) $errors[] = 'Invalid year';

        $model = new ServiceModel();
        $service = $model->findBySlug($service_slug);
        if (!$service) $errors[] = 'Unknown service selected';
        ...
        if (empty($errors)) {
            ...
            $modifier = 1.0;
            if ($age >= 6 && $age <= 10) $modifier = 1.10;
            if ($age > 10) $modifier = 1.20;

            // Example: optionally increase for certain services (per-tire multiply)
            $multiplier = 1;
            if ($service['slug'] === 'tire') {
